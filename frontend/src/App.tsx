...
export default function App() {
  ...
  const progressLabels = state.status === "needs_clarification" ? ["任务提交", "等待补充", "生成图层", "完成交付"] : ["任务提交", "连接服务", "生成图层", "完成交付"];
      ...
      <section className="workspace-grid">
        ...
        <section className="center-column">
          <div className="panel map-panel">
            ...
            <div className="map-stage" ref={mapStageRef}>
              ...
              <div className={`progress-dock progress-dock-${state.status}`} aria-live="polite"><div className="progress-dock-heading"><strong>处理进度</strong><span>{latestLog ? String(latestLog.message || latestLog.content || "") : isWorking ? "等待第一条执行记录" : "提交需求后显示执行进度"}</span></div><div className="progress-steps">{progressLabels.map((label, index) => { const done = state.status === "completed" || index < progressStep; return <div className={`progress-step ${done ? "done" : index === progressStep ? "current" : ""}`} key={label}><span>{done ? "✓" : index + 1}</span>{label}</div>; })}</div></div>
            </div>
          ...
          <div className="panel log-panel">
            <div className="panel-heading"><div><span className="section-kicker">ACTIVITY</span><h2>执行日志</h2></div><span className="log-count">{state.logs.length ? `${state.logs.length} 条记录` : "等待任务"}</span></div>
        ...
        <aside className="panel inspector-panel">
          ...
          <div className="inspector-scroll">
            <section className={`task-status status-card status-card-${state.status}`}><div className="status-card-top"><div><span className="card-label">当前状态</span><strong>{statusLabel(state.status)}</strong></div><span className="status-icon" aria-hidden="true">{state.status === "completed" ? "✓" : state.status === "failed" ? "!" : state.status === "needs_clarification" ? "?" : state.status === "idle" ? "·" : "↗"}</span></div><p>{state.status === "failed" && state.error ? "本轮任务未完成，请根据失败原因处理后重试。" : statusDetail(state.status, state.transportStatus)}</p>{state.traceId && <div className="trace-line"><span>Trace</span><code>{state.traceId}</code></div>}{state.status === "failed" && state.error && <div className="failure-detail" role="alert"><strong>失败原因</strong><p>{state.error}</p><span>修正需求后可继续当前地图，不会覆盖已有成果。</span><button className="retry-button" type="button" onClick={() => setPrompt(latestRequest?.content || "")}>带入原需求</button></div>}{state.transportError && <div className="transport-notice" role="status">{state.transportError}，正在用任务状态同步恢复。</div>}</section>
...
async function openHistory(item: MapRequestSummary, dispatch: Dispatch<WorkbenchAction>, stream: ReturnType<typeof useMapBuildStream>, setGeneratedMaps: (maps: GeneratedMap[]) => void) {
  dispatch({ type: "history_loaded", requestId: item.request_id, status: item.status, viewState: item.view_state, clarification: item.clarification, traceId: item.latest_run?.trace_id, error: item.error_message || item.latest_run?.error_message || null });
  setGeneratedMaps(item.maps || []);
  const [messages, maps, logs] = await Promise.all([mappingApi.messages(item.request_id), mappingApi.generated(item.request_id), mappingApi.logs(item.request_id)]);
