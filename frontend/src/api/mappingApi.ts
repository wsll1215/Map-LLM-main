...
export const mappingApi = {
  async create(prompt: string, operationId = idempotencyKey("map-request")) {
    const result = await apiFetch<{ request_id: number }>("/mapping/api/create-request/", {
      method: "POST",
      headers: { "Idempotency-Key": operationId },
      body: JSON.stringify({ request_text: prompt }),
    });
    return { request_id: result.request_id, title: prompt, status: "pending" as const };
  },
  process: (id: number, operationId = idempotencyKey(`map-process-${id}`)) => apiFetch<{ request_id: number }>("/mapping/api/process-request/", {
    method: "POST",
    headers: { "Idempotency-Key": operationId },
  ...
  continue: (
    id: number,
    message: string,
    operationId = idempotencyKey(`map-message-${id}`),
    messageId = idempotencyKey(`message-${id}`),
  ) => apiFetch<{ request_id: number; stream_after_id?: string }>("/mapping/api/continue-conversation/", {
    method: "POST",
    headers: { "Idempotency-Key": operationId, "X-Message-Id": messageId },
