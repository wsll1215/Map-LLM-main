...
export function MapCanvas({ requestId, layers, status, mapCrs, dataError, dataLoading, onViewportChange, onError, onRetry, onPerformanceMetrics }: { requestId: number | null; layers: LayerPayload[]; status: MapStatus; mapCrs?: string | null; dataError?: string | null; dataLoading?: boolean; onViewportChange?: (bbox: Bbox) => void; onError?: (message: string) => void; onRetry?: () => void; onPerformanceMetrics?: (metrics: PerformanceSnapshot) => void }) {
  const [target, setTarget] = useState<HTMLDivElement | null>(null);
  const [visible, setVisible] = useState<Record<string, boolean>>({});
  useOpenLayersMap(target, requestId, layers, visible, onViewportChange, onError, mapCrs ?? undefined, onPerformanceMetrics);
  ...
  return <div className="map-shell"><div ref={setTarget} className="map-canvas" />
    ...
    {layers.length === 0 && <div className={`map-empty map-empty-${status}`} aria-live="polite" aria-busy={status === "pending" || status === "processing"}><div className="map-empty-mark" aria-hidden="true"><span /><span /><span /></div><div className="map-empty-copy"><strong>{emptyState.title}</strong><p>{emptyState.detail}</p></div>{(status === "pending" || status === "processing") && <div className="map-progress"><span /></div>}</div>}
